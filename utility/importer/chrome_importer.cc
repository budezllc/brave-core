...
void ChromeImporter::ImportPasswords() {
    ...
    LOG(ERROR) << "database.GetAutofillableLogins returned " << success;
    if (success) {
      LOG(ERROR) << "Found " << forms.size() << " autofillable logins";
    ...
    if (success) {
      LOG(ERROR) << "Found " << blacklist.size() << " blacklisted logins";
